...
class SettingsViewController: RootViewController,SearchBarProtocol,BottomProtocol,predicateTableviewProtocol {
    ...
    @IBOutlet weak var bottombatBottomConstraint: NSLayoutConstraint!
    ...
    func setUILayout()
    {
        if UIDevice().userInterfaceIdiom == .phone {
            if (UIScreen.main.nativeBounds.height == 2436) {
                ...
                bottombatBottomConstraint.constant = 0
            }
            else{
                ...
                .responseObject { (response: DataResponse<PredicateSearchData>) -> Void in
                    ...
                    case .success(let data):
                        ...
                        if ((self.predicateSearchArray?.count == 1) && (self.predicateSearchArray![0].item_id == nil))
                        {
                            ...
                            if UIDevice().userInterfaceIdiom == .phone {
                                if (UIScreen.main.nativeBounds.height == 2436) {
                                    self.controller.view.frame = CGRect(x: self.settingsSearchBar.searchInnerView.frame.origin.x, y:self.settingsSearchBar.searchInnerView.frame.origin.y+self.settingsSearchBar.searchInnerView.frame.height+40, width: self.settingsSearchBar.searchInnerView.frame.width, height: CGFloat((self.predicateSearchArray?.count)!*(self.predicateTableHeight)!))
                                }
                                else {
    ...
    func setFontForSettings() {
        ...
        if ( UIScreen.main.bounds.width <= 320 ) {
            ...
            searchTextFont = (settingsSearchBar.searchText.font?.pointSize)!
        }
        if ((LocalizationLanguage.currentAppleLanguage()) == "en") {
